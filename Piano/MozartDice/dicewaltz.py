...
THIS VERSION MODIFIED BY ROBIN NEWMAN TO PLAY THE CONTREDANSES INSTEAD OF THE MINUETS
"""
...
def main():
    ...
    parser.add_argument('-c', '--choice', help='CHOICE=1: generate the standard waltz,       CHOICE=2: generate a contredanse {lilypond mode only}', type=int, default=1)
    ...
    parser.add_argument('-v', '--version', action='version', version=common.CONTREDANSE_VERSION)
    ...
    if export in ('ly', 'lily', 'lilypond'): 
    	if args.choice == 2:
        	from waltzdice import dicelilycontredanse
        	out = dicelilycontredanse.out
        else:
        	from waltzdice import dicelilywaltz
        	out = dicelilywaltz.out
        	
    elif export == 'abc' and args.choice == 1: 
        from waltzdice import diceabc
        out = diceabc.out
    elif export == 'mup' and args.choice == 1: 
        from waltzdice import dicemup
        out = dicemup.out

    else:
    	if args.choice == 2:
    		redprint ('Contradanse only available for lilypond output!', nc)
    	else:	
    ...
    if args.outfile:
        ...
        with open(outfile, 'w') as f:
            ...
            if args.choice == 2:
		greenprint ('Contredanse written to {}'.format(outfile), nc)	
	    else:
